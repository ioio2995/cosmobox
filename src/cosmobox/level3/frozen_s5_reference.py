"""Loader/adapter for the frozen Level3 S5 reference campaign
(results/level3/level3-s5-truncation-extension-v1/), the sole normative
source of S=5 data for the Level3 S6 campaign (lot L3-W).

Mirrors cosmobox.level3.frozen_s4_reference (the S4 loader, L3-P) exactly,
one layer up: every function here reads already-persisted,
already-versioned artifacts (frozen by lot L3-S,
results/level3/level3-s5-truncation-extension-v1/SHA256SUMS) and never
...
FrozenS5ReferenceIntegrityError -- there is no fallback and no silent
recomputation path.

reconstruct_case_execution_result rebuilds a
cosmobox.level3.execution.CaseExecutionResult byte-faithful to the
persisted S=5 case-result artifact, suitable for
...
(None), exactly as the Level2 and S4 loaders do.

frozen_metric_comparison exposes the already-aggregated S2/S3/S4/S5
values (delta_hl_s2..s5, direction_s2..s5, t_x_34, t_x_45, c_x_34, d_x_34,
c_x_45, d_x_45, ...) directly from the frozen S5 campaign-summary.json --
these are the S5-side inputs the S6 campaign-summary needs and must never
recompute.
"""
...
from cosmobox.level3.s5_serialization import validate_campaign_summary_document, validate_case_result_document

LEVEL3_S5_REFERENCE_CAMPAIGN_ID = "level3-s5-truncation-extension-v1"
LEVEL3_S5_REFERENCE_MANIFEST_FINGERPRINT = "3160a8e6e0f9ae4a21c027a865e4d56527303ad644e3f45137baa2120f0c5d04"
LEVEL3_S5_REFERENCE_REPOSITORY = "ioio2995/cosmobox"
LEVEL3_S5_REFERENCE_REPOSITORY_COMMIT = "782f29eb9dcbc21ca2168e109997b0f59a12402f"
LEVEL3_S5_REFERENCE_FROZEN_PREREGISTRATION_COMMIT = "496ba9484a6d7df9beb738607a5ffe23a75219ad"
"""Pinned literal identity of the frozen Level3 S5 campaign -- the same
values as experiments.level3.s6_manifest.Level3S5ReferenceIdentity's own
...
LEVEL3_S5_REFERENCE_GEOMETRIES = ("triangle", "ring4", "ring5")
LEVEL3_S5_REFERENCE_SPIN = 5
LEVEL3_S5_REFERENCE_DIMENSIONS: dict[str, int] = {"triangle": 208, "ring4": 712, "ring5": 2512}

_DEFAULT_ROOT = Path(__file__).resolve().parents[3] / "results" / "level3" / LEVEL3_S5_REFERENCE_CAMPAIGN_ID
_SHA256SUMS_NAME = "SHA256SUMS"


class FrozenS5ReferenceIntegrityError(ValueError):
    """Raised for any missing file, SHA-256 mismatch, schema failure, or
    provenance mismatch in the frozen Level3 S5 reference. No fallback
    and no recomputation is ever attempted from this branch."""


def frozen_s5_reference_root(root: Path | None = None) -> Path:
    return Path(root) if root is not None else _DEFAULT_ROOT


def _case_artifact_path(geometry: str, *, root: Path) -> Path:
    return root / "cases" / f"{geometry}-S{LEVEL3_S5_REFERENCE_SPIN}.json"


def verify_sha256sums(root: Path | None = None) -> None:
    """Reads SHA256SUMS (itself never hashed) and verifies every listed
    file hashes exactly as recorded. Raises FrozenS5ReferenceIntegrityError
    on a missing SHA256SUMS, a missing listed file, or any hash
    mismatch."""
    resolved_root = frozen_s5_reference_root(root)
    sums_path = resolved_root / _SHA256SUMS_NAME
    if not sums_path.exists():
        raise FrozenS5ReferenceIntegrityError(f"missing integrity record: {sums_path}")
    ...
    for line in sums_path.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            raise FrozenS5ReferenceIntegrityError(f"malformed SHA256SUMS line: {line!r}") from None
        relative_path = raw_path.lstrip(" *")
        target = resolved_root / relative_path
        if not target.exists():
            raise FrozenS5ReferenceIntegrityError(
        ...
        if actual_hash != expected_hash:
            raise FrozenS5ReferenceIntegrityError(
                ...
                "-- refusing to use a frozen Level3 S5 reference artifact whose bytes have changed"
            )


def _load_json(path: Path) -> dict:
    if not path.exists():
        raise FrozenS5ReferenceIntegrityError(f"missing frozen Level3 S5 reference artifact: {path}")
    try:
        return json.loads(path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise FrozenS5ReferenceIntegrityError(
            f"frozen Level3 S5 reference artifact is not valid JSON: {path}: {exc}"
        ) from exc
...
def _verify_pinned_identity(document: dict, *, path: Path) -> None:
    ...
    checks = (
        ("campaign_id", LEVEL3_S5_REFERENCE_CAMPAIGN_ID),
        ("manifest_fingerprint", LEVEL3_S5_REFERENCE_MANIFEST_FINGERPRINT),
        ("repository", LEVEL3_S5_REFERENCE_REPOSITORY),
        ("repository_commit", LEVEL3_S5_REFERENCE_REPOSITORY_COMMIT),
        ("frozen_preregistration_commit", LEVEL3_S5_REFERENCE_FROZEN_PREREGISTRATION_COMMIT),
    )
    for key, expected_value in checks:
        if key in document and document[key] != expected_value:
            raise FrozenS5ReferenceIntegrityError(
                f"{path}: {key}={document[key]!r} does not match the pinned frozen Level3 S5 reference "
                f"identity {expected_value!r}"
            )


def load_manifest_snapshot(*, root: Path | None = None) -> dict:
    """The frozen S5 manifest.json snapshot -- provenance-checked, never
    schema-validated here (it is a raw snapshot of Level3S5Manifest.raw,
...
def load_campaign_summary(*, root: Path | None = None) -> dict:
    """The frozen S5 campaign-summary.json -- schema-validated against
    schemas/level3/s5-campaign-summary-v1.schema.json (reused unchanged
    via cosmobox.level3.s5_serialization) and provenance-checked."""
    resolved_root = frozen_s5_reference_root(root)
    ...
    if geometries != set(LEVEL3_S5_REFERENCE_GEOMETRIES):
        raise FrozenS5ReferenceIntegrityError(
            f"{path}: geometries {sorted(geometries)} do not match the expected "
            f"{sorted(LEVEL3_S5_REFERENCE_GEOMETRIES)}"
        )
    return document


def load_case_result(geometry: str, *, root: Path | None = None) -> dict:
    """One frozen S5 case-result document -- schema-validated against
    schemas/level3/s5-case-result-v1.schema.json (reused unchanged),
    provenance-checked, and cross-checked for geometry/spin/dimension/
    full-spectrum invariants against the frozen L3-N/L3-R reference."""
    if geometry not in LEVEL3_S5_REFERENCE_DIMENSIONS:
        raise ValueError(
            f"geometry={geometry!r} is not one of the three frozen Level3 S5 reference cases "
            f"{sorted(LEVEL3_S5_REFERENCE_DIMENSIONS)}"
        )
    resolved_root = frozen_s5_reference_root(root)
    ...
    if document["geometry"] != geometry or document["spin"] != LEVEL3_S5_REFERENCE_SPIN:
        raise FrozenS5ReferenceIntegrityError(
            f"{path}: (geometry, spin)=({document['geometry']!r}, {document['spin']!r}) does not match "
            f"the requested ({geometry!r}, {LEVEL3_S5_REFERENCE_SPIN!r})"
        )
    expected_dimension = LEVEL3_S5_REFERENCE_DIMENSIONS[geometry]
    ...
    if document["full_spectrum"] is not True:
        raise FrozenS5ReferenceIntegrityError(f"{path}: full_spectrum must be true")
    if document["solver_method"] != "dense":
        raise FrozenS5ReferenceIntegrityError(f"{path}: solver_method must be 'dense'")
    if document["window_truncated"] is not False:
        raise FrozenS5ReferenceIntegrityError(f"{path}: window_truncated must be false")
    if document["partial_subspace_count"] != 0:
        raise FrozenS5ReferenceIntegrityError(f"{path}: partial_subspace_count must be 0")
...
def verify_frozen_s5_reference(*, root: Path | None = None) -> None:
    """The single entry point a campaign runner should call before using
    any frozen S5 data: verifies SHA256SUMS, then loads and validates the
    manifest snapshot, the campaign-summary, and all three case-result
    artifacts. Raises FrozenS5ReferenceIntegrityError on the first
    ...
    for geometry in LEVEL3_S5_REFERENCE_DIMENSIONS:
        load_case_result(geometry, root=root)


# ---------------------------------------------------------------------------
# Reconstruction: frozen S5 case-result JSON -> cosmobox.level3.execution.
...
def reconstruct_case_execution_result(geometry: str, *, root: Path | None = None) -> CaseExecutionResult:
    """Rebuild a Level3 CaseExecutionResult for a frozen S=5 case,
    ...
    return CaseExecutionResult(
        spec=CaseSpec(geometry=geometry, spin=LEVEL3_S5_REFERENCE_SPIN),
...
# ---------------------------------------------------------------------------
# Already-aggregated S2/S3/S4/S5 values, read directly (never recomputed).
# ---------------------------------------------------------------------------
...
def frozen_metric_comparison(geometry: str, metric: str, *, root: Path | None = None) -> dict:
    """The already-aggregated S2/S3/S4/S5 comparison for one geometry/
    metric (primary metrics carry delta_hl_s2/s3/s4/s5, direction_s2/s3/
    s4/s5, primary_s4_s5_taxonomy, t_x_34/t_x_45/r_delta_x_45_34,
    c_x_34/d_x_34/c_x_45/d_x_45/r_d_x_45_34; control metrics carry
    delta_hl_s2/s3/s4/s5 only), read directly from the frozen S5
    ...
    if geometry not in LEVEL3_S5_REFERENCE_GEOMETRIES:
        raise ValueError(f"geometry must be one of {LEVEL3_S5_REFERENCE_GEOMETRIES}, got {geometry!r}")
    ...
    raise FrozenS5ReferenceIntegrityError(f"geometry {geometry!r} not found in the frozen Level3 S5 campaign-summary")
