"""Unit tests for scripts.level3_s5_campaign.outputs (lot L3-P).
Filesystem-only for the lower-level primitives; case-result read/write
tests build a real, schema-valid document via
cosmobox.level3.s5_serialization.case_result_payload from
...
from cosmobox.level3 import s5_serialization
from cosmobox.level3.execution import CaseExecutionResult, CaseSpec
from scripts.level3_s5_campaign.outputs import (
...
from scripts.level3_s5_campaign.provenance import REPOSITORY_IDENTITY
...
def _case_result_document(geometry: str, *, provenance: dict = _PROVENANCE) -> dict:
    ...
    result = CaseExecutionResult(
        spec=CaseSpec(geometry=geometry, spin=5),
        dimension=1,
        group_count=1,
        entries=entries,
    ...
    return s5_serialization.case_result_payload(result, **provenance)
...
def test_campaign_output_dir_and_summary_path_layout(tmp_path):
    output_dir = campaign_output_dir(tmp_path, "level3-s5-truncation-extension-v1")
    assert output_dir == tmp_path / "level3-s5-truncation-extension-v1"
    summary_path = campaign_summary_path(tmp_path, "level3-s5-truncation-extension-v1")
    assert summary_path == output_dir / "campaign-summary.json"


def test_write_campaign_summary_is_atomic_and_readable(tmp_path):
    document = {"schema_version": "level3-s5-campaign-summary-v1", "campaign_status": "COMPLETE"}
    path = write_campaign_summary(tmp_path, "level3-s5-truncation-extension-v1", document)
    assert path == campaign_summary_path(tmp_path, "level3-s5-truncation-extension-v1")
    assert json.loads(path.read_text()) == document


def test_write_campaign_summary_refuses_to_overwrite_existing(tmp_path):
    document = {"schema_version": "level3-s5-campaign-summary-v1", "campaign_status": "COMPLETE"}
    write_campaign_summary(tmp_path, "level3-s5-truncation-extension-v1", document)
...
def test_campaign_manifest_path_layout(tmp_path):
    path = campaign_manifest_path(tmp_path, "level3-s5-truncation-extension-v1")
    assert path == campaign_output_dir(tmp_path, "level3-s5-truncation-extension-v1") / "manifest.json"


def test_write_campaign_manifest_is_atomic_and_readable(tmp_path):
    raw = {"campaign_id": "level3-s5-truncation-extension-v1", "cases": []}
    path = write_campaign_manifest(tmp_path, "level3-s5-truncation-extension-v1", raw)
    assert path == campaign_manifest_path(tmp_path, "level3-s5-truncation-extension-v1")
    assert json.loads(path.read_text()) == raw


def test_write_campaign_manifest_refuses_to_overwrite_existing(tmp_path):
    raw = {"campaign_id": "level3-s5-truncation-extension-v1", "cases": []}
    write_campaign_manifest(tmp_path, "level3-s5-truncation-extension-v1", raw)
...
def test_write_campaign_manifest_refuses_to_overwrite_even_a_different_manifest(tmp_path):
    original = {"campaign_id": "level3-s5-truncation-extension-v1", "cases": ["old"]}
    write_campaign_manifest(tmp_path, "level3-s5-truncation-extension-v1", original)
    different = {"campaign_id": "level3-s5-truncation-extension-v1", "cases": ["new"]}
    with pytest.raises(CampaignManifestAlreadyExists):
        write_campaign_manifest(tmp_path, "level3-s5-truncation-extension-v1", different)
    assert json.loads(campaign_manifest_path(tmp_path, "level3-s5-truncation-extension-v1").read_text()) == original
...
def test_case_artifact_path_layout(tmp_path):
    path = case_artifact_path(tmp_path, "level3-s5-truncation-extension-v1", "triangle", 5)
    assert path == campaign_output_dir(tmp_path, "level3-s5-truncation-extension-v1") / "cases" / "triangle-S5.json"


def test_write_case_result_is_atomic_and_readable(tmp_path):
    document = _case_result_document("triangle")
    path = write_case_result(tmp_path, "level3-s5-truncation-extension-v1", "triangle", 5, document)
    assert path == case_artifact_path(tmp_path, "level3-s5-truncation-extension-v1", "triangle", 5)
    assert json.loads(path.read_text()) == document


def test_write_case_result_refuses_to_overwrite_existing(tmp_path):
    document = _case_result_document("triangle")
    write_case_result(tmp_path, "level3-s5-truncation-extension-v1", "triangle", 5, document)
...
def test_load_and_verify_case_result_round_trips_a_valid_artifact(tmp_path):
    ...
    write_case_result(tmp_path, "level3-s5-truncation-extension-v1", "ring4", 5, document)
    loaded = load_and_verify_case_result(
        tmp_path, "level3-s5-truncation-extension-v1", "ring4", 5,
...
def test_load_and_verify_case_result_raises_on_schema_invalid_artifact(tmp_path):
    ...
    atomic_write_json(case_artifact_path(tmp_path, "level3-s5-truncation-extension-v1", "triangle", 5), document)
    with pytest.raises(CaseArtifactIntegrityError):
        load_and_verify_case_result(
            tmp_path, "level3-s5-truncation-extension-v1", "triangle", 5,
            manifest_fingerprint=_PROVENANCE["manifest_fingerprint"],
            repository_commit=_PROVENANCE["repository_commit"],
        )
...
def test_load_and_verify_case_result_raises_on_campaign_id_mismatch(tmp_path):
    ...
    with pytest.raises(CaseArtifactIntegrityError):
        load_and_verify_case_result(
            tmp_path, "other-campaign-id", "triangle", 5,
            manifest_fingerprint=_PROVENANCE["manifest_fingerprint"],
            repository_commit=_PROVENANCE["repository_commit"],
        )
