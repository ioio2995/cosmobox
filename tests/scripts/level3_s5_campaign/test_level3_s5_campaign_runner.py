"""Unit/integration tests for scripts.level3_s5_campaign.runner (lot L3-P).

execution.run_case is ALWAYS monkeypatched to a synthetic factory here --
no real diagonalization, no real S=5 Hamiltonian, anywhere in this file.
The frozen S=4 reference (cosmobox.level3.frozen_s4_reference) and the
frozen S=2/S=3 reference it embeds are the real, versioned data -- read-
only, never modified, never faked. execution.compare_spin_pair is the
real function, called on the synthetic (but fully valid) S5
CaseExecutionResult and the real reconstructed S4 CaseExecutionResult.
...
from experiments.level3.s5_manifest import load_manifest
from scripts.level3_s5_campaign import runner
from scripts.level3_s5_campaign.outputs import campaign_manifest_path, campaign_summary_path, case_artifact_path
from scripts.level3_s5_campaign.provenance import REPOSITORY_IDENTITY, CampaignProvenance
from scripts.level3_s5_campaign.runner import CampaignAlreadyExists, run_campaign
...
def _synthetic_gate_reference(monkeypatch):
    # The real L3-N dimensions (208/712/2512) would require a real
    ...
    monkeypatch.setattr(runner.gates, "EXPECTED_S5_DIMENSIONS", dict(_SYNTHETIC_DIMENSIONS))
...
# ---------------------------------------------------------------------------
# End-to-end synthetic integration -- REAL frozen S4/S3/S2 reference, FAKE S5
# ---------------------------------------------------------------------------
...
def test_run_campaign_produces_full_artifact_tree(tmp_path, manifest, _fake_run_case):
    ...
    for geometry in ("triangle", "ring4", "ring5"):
        assert case_artifact_path(tmp_path, manifest.campaign_id, geometry, 5).exists()
    ...
    case_document = json.loads(case_artifact_path(tmp_path, manifest.campaign_id, "triangle", 5).read_text())
    assert case_document["geometry"] == "triangle"
    assert case_document["spin"] == 5
...
def test_run_campaign_invokes_run_case_exactly_three_times_in_manifest_order(tmp_path, manifest, _fake_run_case):
    ...
    assert all(spec.spin == 5 for spec in _fake_run_case)
...
def test_run_campaign_calls_compare_spin_pair_exactly_three_times(tmp_path, manifest, _fake_run_case, _compare_spin_pair_spy):
    ...
    # result_a is always the reconstructed S4 reference, result_b the S5 result
    assert all(result_a.spec.spin == 4 for result_a, _ in _compare_spin_pair_spy)
    assert all(result_b.spec.spin == 5 for _, result_b in _compare_spin_pair_spy)


# ---------------------------------------------------------------------------
# Ordering: plan 3/3 before any comparison; no comparison before all three
# S5 cases are persisted
# ---------------------------------------------------------------------------
...
def test_run_campaign_never_compares_before_all_three_s5_cases_are_persisted(tmp_path, manifest, monkeypatch):
    events: list[str] = []
    real = compare_spin_pair
...
def test_run_campaign_writes_campaign_summary_last(tmp_path, manifest, _fake_run_case):
    ...
    case_paths = [case_artifact_path(tmp_path, manifest.campaign_id, g, 5) for g in ("triangle", "ring4", "ring5")]
...
def test_run_campaign_leaves_no_summary_when_branch_verification_fails(tmp_path, manifest, monkeypatch):
    from scripts.level3_s5_campaign.provenance import ProvenanceResolutionFailure
    ...
    with pytest.raises(ProvenanceResolutionFailure):
        run_campaign(tmp_path, manifest=manifest)
    assert not campaign_summary_path(tmp_path, manifest.campaign_id).exists()
    assert not case_artifact_path(tmp_path, manifest.campaign_id, "triangle", 5).exists()
...
# ---------------------------------------------------------------------------
# No real S=5 physical function is ever reachable from the runner
# ---------------------------------------------------------------------------


def test_run_campaign_never_calls_a_real_physical_s5_primitive(tmp_path, manifest, monkeypatch, _fake_run_case):
