"""Unit tests for scripts.level3_s6_campaign.planning (lot L3-W). Uses the
...
from scripts.level3_s6_campaign.planning import EXPECTED_CASES, plan_campaign


def test_plan_campaign_produces_exactly_three_s6_cases():
    manifest = load_manifest()
    planned = plan_campaign(manifest)
    assert len(planned) == 3
    assert {(spec.geometry, spec.spin) for spec in planned} == set(EXPECTED_CASES)
    assert all(spec.spin == 6 for spec in planned)
...
def test_plan_campaign_rejects_spin_7():
    manifest = load_manifest()
    bad_case = manifest.cases[0]
    object.__setattr__(bad_case, "spin", 7)
...
def test_plan_campaign_rejects_an_extra_geometry():
    manifest = load_manifest()
    from experiments.level3.s6_manifest import Level3S6ManifestCase

    extra = Level3S6ManifestCase(geometry="triangle", spin=6)
