"""Unit tests for scripts.level3_s6_campaign.gates (lot L3-W). Synthetic
...
from scripts.level3_s6_campaign.gates import CampaignGateFailure, CaseGateFailure, validate_campaign_completeness, validate_case_result
...
def _result(geometry: str, dimension: int, *, spin: int = 6) -> CaseExecutionResult:
    n = dimension
...
# ---------------------------------------------------------------------------
# validate_case_result: dimension against the L3-U reference
# ---------------------------------------------------------------------------


def test_validate_case_result_accepts_triangle_248():
    validate_case_result(_result("triangle", 248))  # must not raise


def test_validate_case_result_accepts_ring4_852():
    validate_case_result(_result("ring4", 852))  # must not raise


def test_validate_case_result_accepts_ring5_3016():
    validate_case_result(_result("ring5", 3016))  # must not raise


def test_validate_case_result_rejects_wrong_dimension():
    with pytest.raises(CaseGateFailure):
        validate_case_result(_result("triangle", 249))


def test_validate_case_result_rejects_spin_other_than_6():
    with pytest.raises(CaseGateFailure):
        validate_case_result(_result("triangle", 248, spin=5))


def test_validate_case_result_rejects_unknown_geometry():
    result = _result("triangle", 248)
...
def test_validate_case_result_rejects_q_coverage_gap():
    # Multiplicities sum to exactly 248 (the triangle S6 reference
    # dimension) so the dimension check passes and the q-coverage check
    # is isolated: q_end=0.3 then q_start=0.35 is a genuine gap.
    entries = (
        _entry(0.0, 0.3, 1.0, multiplicity=80),
        _entry(0.35, 0.6, 2.0, multiplicity=84),
        _entry(0.6, 1.0, 3.0, multiplicity=84),
    )
    analyses = {m: orchestration.analyze_case_metric(entries, m) for m in orchestration.ALL_METRICS}
    result = CaseExecutionResult(
        spec=CaseSpec(geometry="triangle", spin=6), dimension=248, group_count=3, entries=entries,
        m_tt_analysis=analyses["M_TT"], r_eff_analysis=analyses["R_eff"],
        a_qq_analysis=analyses["A_QQ"], m_qq_analysis=analyses["M_QQ"],
    )
    with pytest.raises(CaseGateFailure):
...
def _comparison(geometry: str):
    result_a = _result(geometry, 4, spin=5)
    result_b = _result(geometry, 5, spin=6)
...
def test_validate_campaign_completeness_accepts_exactly_three_cases_and_comparisons():
    results = [_result("triangle", 248), _result("ring4", 852), _result("ring5", 3016)]
...
def test_validate_campaign_completeness_rejects_missing_case():
    results = [_result("triangle", 248), _result("ring4", 852)]
    comparisons = [_comparison("triangle"), _comparison("ring4"), _comparison("ring5")]
...
def test_validate_campaign_completeness_rejects_duplicate_geometry():
    results = [_result("triangle", 248), _result("triangle", 248), _result("ring5", 3016)]
    comparisons = [_comparison("triangle"), _comparison("ring4"), _comparison("ring5")]
    with pytest.raises(CampaignGateFailure):
        validate_campaign_completeness(results, comparisons)
