"""Unit/integration tests for scripts.level3_s6_campaign.runner (lot L3-W).

execution.run_case is ALWAYS monkeypatched to a synthetic factory here --
no real diagonalization, no real S=6 Hamiltonian, anywhere in this file.
The frozen S=5 reference (cosmobox.level3.frozen_s5_reference) and the
frozen S=2/S=3/S=4 references it embeds are the real, versioned data --
read-only, never modified, never faked. execution.compare_spin_pair is
the real function, called on the synthetic (but fully valid) S6
CaseExecutionResult and the real reconstructed S5 CaseExecutionResult.
...
from experiments.level3.s6_manifest import load_manifest
from scripts.level3_s6_campaign import runner
from scripts.level3_s6_campaign.outputs import campaign_manifest_path, campaign_summary_path, case_artifact_path
from scripts.level3_s6_campaign.provenance import REPOSITORY_IDENTITY, CampaignProvenance
from scripts.level3_s6_campaign.runner import CampaignAlreadyExists, run_campaign
...
def _synthetic_gate_reference(monkeypatch):
    # The real L3-U dimensions (248/852/3016) would require a real
    ...
    monkeypatch.setattr(runner.gates, "EXPECTED_S6_DIMENSIONS", dict(_SYNTHETIC_DIMENSIONS))
...
# ---------------------------------------------------------------------------
# End-to-end synthetic integration -- REAL frozen S5/S4/S3/S2 reference, FAKE S6
# ---------------------------------------------------------------------------
...
def test_run_campaign_produces_full_artifact_tree(tmp_path, manifest, _fake_run_case):
    ...
    for geometry in ("triangle", "ring4", "ring5"):
        assert case_artifact_path(tmp_path, manifest.campaign_id, geometry, 6).exists()
    ...
    case_document = json.loads(case_artifact_path(tmp_path, manifest.campaign_id, "triangle", 6).read_text())
    assert case_document["geometry"] == "triangle"
    assert case_document["spin"] == 6
...
def test_run_campaign_invokes_run_case_exactly_three_times_in_manifest_order(tmp_path, manifest, _fake_run_case):
    ...
    assert all(spec.spin == 6 for spec in _fake_run_case)
...
def test_run_campaign_calls_compare_spin_pair_exactly_three_times(tmp_path, manifest, _fake_run_case, _compare_spin_pair_spy):
    ...
    # result_a is always the reconstructed S5 reference, result_b the S6 result
    assert all(result_a.spec.spin == 5 for result_a, _ in _compare_spin_pair_spy)
    assert all(result_b.spec.spin == 6 for _, result_b in _compare_spin_pair_spy)


# ---------------------------------------------------------------------------
# Ordering: plan 3/3 before any comparison; no comparison before all three
# S6 cases are persisted
# ---------------------------------------------------------------------------
...
def test_run_campaign_never_compares_before_all_three_s6_cases_are_persisted(tmp_path, manifest, monkeypatch):
    events: list[str] = []
    real = compare_spin_pair
...
def test_run_campaign_writes_campaign_summary_last(tmp_path, manifest, _fake_run_case):
    ...
    case_paths = [case_artifact_path(tmp_path, manifest.campaign_id, g, 6) for g in ("triangle", "ring4", "ring5")]
...
def test_run_campaign_leaves_no_summary_when_branch_verification_fails(tmp_path, manifest, monkeypatch):
    from scripts.level3_s6_campaign.provenance import ProvenanceResolutionFailure
    ...
    with pytest.raises(ProvenanceResolutionFailure):
        run_campaign(tmp_path, manifest=manifest)
    assert not campaign_summary_path(tmp_path, manifest.campaign_id).exists()
    assert not case_artifact_path(tmp_path, manifest.campaign_id, "triangle", 6).exists()
...
# ---------------------------------------------------------------------------
# No real S=6 physical function is ever reachable from the runner
# ---------------------------------------------------------------------------


def test_run_campaign_never_calls_a_real_physical_s6_primitive(tmp_path, manifest, monkeypatch, _fake_run_case):
