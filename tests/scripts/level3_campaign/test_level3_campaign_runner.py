"""Unit/integration tests for scripts.level3_campaign.runner (lot L3-I).

execution.run_case is ALWAYS monkeypatched to a synthetic factory here --
no real diagonalization, no real S=4 Hamiltonian, anywhere in this file.
The frozen S=2/S=3 reference (cosmobox.level3.frozen_reference) is the
real, versioned results/level2/level2-energy-regime-v1/ data -- read-only,
never modified, never faked (there is no need to: it is small, real, and
already integrity-checked by tests/level3/test_level3_frozen_reference.py).
execution.compare_spin_pair is the real function, called on the synthetic
(but fully valid) S4 CaseExecutionResult and the real reconstructed S3
...
from experiments.level3.manifest import EXPECTED_S4_DIMENSIONS, load_manifest
from scripts.level3_campaign import runner
from scripts.level3_campaign.outputs import campaign_manifest_path, campaign_summary_path, case_artifact_path
from scripts.level3_campaign.provenance import REPOSITORY_IDENTITY, CampaignProvenance
from scripts.level3_campaign.runner import CampaignAlreadyExists, run_campaign
...
def _synthetic_case_result(spec: CaseSpec, *, n: int = 3) -> CaseExecutionResult:
...
def _synthetic_gate_reference(monkeypatch):
    # The real L3-C dimensions (168/572/2008) would require a real
    ...
    # synthetic dimension (3) this file's fake run_case actually returns.
    monkeypatch.setattr(
        runner.gates, "EXPECTED_S4_DIMENSIONS", {"triangle": 3, "ring4": 3, "ring5": 3}
    )
...
# ---------------------------------------------------------------------------
# End-to-end synthetic integration -- REAL frozen S2/S3 reference, FAKE S4
# ---------------------------------------------------------------------------
...
def test_run_campaign_produces_full_artifact_tree(tmp_path, manifest, _fake_run_case):
    ...
    for geometry in ("triangle", "ring4", "ring5"):
        assert case_artifact_path(tmp_path, manifest.campaign_id, geometry, 4).exists()
    ...
    case_document = json.loads(case_artifact_path(tmp_path, manifest.campaign_id, "triangle", 4).read_text())
    assert case_document["geometry"] == "triangle"
    assert case_document["spin"] == 4
...
def test_run_campaign_invokes_run_case_exactly_three_times_in_manifest_order(tmp_path, manifest, _fake_run_case):
    ...
    assert all(spec.spin == 4 for spec in _fake_run_case)
...
def test_run_campaign_calls_compare_spin_pair_exactly_three_times(tmp_path, manifest, _fake_run_case, _compare_spin_pair_spy):
    ...
    # result_a is always the reconstructed S3 reference, result_b the S4 result
    assert all(result_a.spec.spin == 3 for result_a, _ in _compare_spin_pair_spy)
    assert all(result_b.spec.spin == 4 for _, result_b in _compare_spin_pair_spy)


# ---------------------------------------------------------------------------
# Ordering: plan 3/3 before any comparison; no comparison before all three
# S4 cases are persisted
# ---------------------------------------------------------------------------
...
def test_run_campaign_never_compares_before_all_three_s4_cases_are_persisted(tmp_path, manifest, monkeypatch):
    events: list[str] = []
    real = compare_spin_pair
...
def test_run_campaign_writes_campaign_summary_last(tmp_path, manifest, _fake_run_case):
    ...
    case_paths = [case_artifact_path(tmp_path, manifest.campaign_id, g, 4) for g in ("triangle", "ring4", "ring5")]
...
def test_run_campaign_leaves_no_summary_when_branch_verification_fails(tmp_path, manifest, monkeypatch):
    from scripts.level3_campaign.provenance import ProvenanceResolutionFailure
    ...
    with pytest.raises(ProvenanceResolutionFailure):
        run_campaign(tmp_path, manifest=manifest)
    assert not campaign_summary_path(tmp_path, manifest.campaign_id).exists()
    assert not case_artifact_path(tmp_path, manifest.campaign_id, "triangle", 4).exists()
...
def test_run_campaign_cannot_overwrite_manifest_of_a_partial_old_campaign(tmp_path, manifest, _fake_run_case):
    ...
    # one case artifact exist, but no campaign-summary.json. A fresh
    # run_campaign attempt must still refuse (NO_IMPLICIT_RESUME), never
    # treating this as resumable.
    campaign_manifest_path(tmp_path, manifest.campaign_id).parent.mkdir(parents=True, exist_ok=True)
...
# ---------------------------------------------------------------------------
# No real S=4 physical function is ever reachable from the runner
# ---------------------------------------------------------------------------


def test_run_campaign_never_calls_a_real_physical_s4_primitive(tmp_path, manifest, monkeypatch, _fake_run_case):
    # run_case is the ONLY entry point the runner may call for physics;
    # every lower-level primitive must remain untouched by this module.
