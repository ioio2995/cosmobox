"""Unit tests for experiments.level3.s5_manifest (lot L3-P-S5-CAMPAIGN-INFRASTRUCTURE).

Exercises the real, frozen experiments/level3/preregistered-s5-manifest-v1.json
...
the historical S4 manifest fingerprint is unaffected by this module's
existence (experiments/level3/manifest.py is never imported or modified
here).
"""
...
from experiments.level3.s5_manifest import (
    CONTROL_METRICS,
    EXPECTED_S5_DIMENSIONS,
    GEOMETRIES,
    PRIMARY_METRICS,
    SPIN,
...
def test_load_manifest_loads_and_validates_the_real_frozen_manifest():
    ...
    assert isinstance(manifest, Level3S5Manifest)
    assert manifest.manifest_version == "level3-s5-reference-v1"
    assert manifest.frozen_preregistration_commit == "496ba9484a6d7df9beb738607a5ffe23a75219ad"
...
def test_frozen_manifest_expected_s5_dimensions_and_capability():
    manifest = load_manifest()
    assert manifest.expected_s5_dimensions == EXPECTED_S5_DIMENSIONS == {"triangle": 208, "ring4": 712, "ring5": 2512}
    assert manifest.validated_dense_capability == VALIDATED_DENSE_CAPABILITY == 2512
...
def test_frozen_manifest_forbids_s6():
    manifest = load_manifest()
    assert "S6" in manifest.forbidden_extensions
...
def test_fingerprint_changes_when_normative_content_changes():
    ...
    mutated["campaign_id"] = "level3-s5-truncation-extension-v1-mutated"
    assert parse_manifest(raw).fingerprint != parse_manifest(mutated).fingerprint


def test_s4_historical_fingerprint_unchanged_by_s5_manifest_module():
    ...
    # produced, unaffected by s5_manifest.py's existence.
...
def test_schema_rejects_spin_6(tmp_path):
    raw = _raw_manifest()
    raw["cases"][0]["spin"] = 6
    path = tmp_path / "manifest.json"
    path.write_text(json.dumps(raw))
...
def test_level3_s5_manifest_case_rejects_out_of_contract_geometry():
    with pytest.raises(ValueError):
        Level3S5ManifestCase(geometry="hexagon", spin=5)


def test_level3_s5_manifest_case_rejects_spin_other_than_5():
    with pytest.raises(ValueError):
        Level3S5ManifestCase(geometry="triangle", spin=6)
    with pytest.raises(ValueError):
        Level3S5ManifestCase(geometry="triangle", spin=4)
