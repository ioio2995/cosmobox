"""Unit tests for experiments.level3.s7_manifest (lot L3-AC-S7-CAMPAIGN-INFRASTRUCTURE).

Exercises the real, frozen experiments/level3/preregistered-s7-manifest-v1.json
...
the historical S4/S5/S6 manifest fingerprints are unaffected by this
module's existence (experiments/level3/manifest.py, s5_manifest.py, and
s6_manifest.py are never imported or modified here).
"""
...
from experiments.level3.s7_manifest import (
    CONTROL_METRICS,
    EXPECTED_S7_DIMENSIONS,
    GEOMETRIES,
    PRIMARY_METRICS,
    SPIN,
    ...
    Level3S6ReferenceIdentity,
    Level3S7Manifest,
    Level3S7ManifestCase,
    load_manifest,
    load_manifest_json,
    parse_manifest,
)
...
S6_HISTORICAL_FINGERPRINT = "2e0f06b69139abeb0b225a3d2b715242d6a0c26d2d1067b789f26b4829f847ca"
...
def test_load_manifest_loads_and_validates_the_real_frozen_manifest():
    ...
    assert isinstance(manifest, Level3S7Manifest)
    assert manifest.manifest_version == "level3-s7-reference-v1"
    assert manifest.frozen_preregistration_commit == "cc0ffdadbeb7d40fc443ab11150c9f47e3e12776"
...
def test_frozen_manifest_pins_the_level3_s6_reference_identity():
    manifest = load_manifest()
    assert manifest.level3_s6_reference.campaign_id == "level3-s6-truncation-extension-v1"
    assert manifest.level3_s6_reference.manifest_fingerprint == S6_HISTORICAL_FINGERPRINT
    assert manifest.level3_s6_reference.repository_commit == "55ea89be3a744953ff677754ff2ca22dbf116428"
    assert manifest.level3_s6_reference.frozen_preregistration_commit == "1c19a01e051c3df6f547afe0816a795989fdac3b"


def test_frozen_manifest_expected_s7_dimensions_and_capability():
    manifest = load_manifest()
    assert manifest.expected_s7_dimensions == EXPECTED_S7_DIMENSIONS == {"triangle": 288, "ring4": 992, "ring5": 3520}
    assert manifest.validated_dense_capability == VALIDATED_DENSE_CAPABILITY == 3520
...
def test_frozen_manifest_forbids_s8():
    manifest = load_manifest()
    assert "S8" in manifest.forbidden_extensions
...
def test_fingerprint_changes_when_normative_content_changes():
    ...
    mutated["campaign_id"] = "level3-s7-truncation-extension-v1-mutated"
    assert parse_manifest(raw).fingerprint != parse_manifest(mutated).fingerprint


def test_s4_historical_fingerprint_unchanged_by_s7_manifest_module():
...
def test_s5_historical_fingerprint_unchanged_by_s7_manifest_module():
...
def test_s6_historical_fingerprint_unchanged_by_s7_manifest_module():
    from experiments.level3.s6_manifest import load_manifest as load_s6_manifest

    s6_manifest = load_s6_manifest()
    assert s6_manifest.fingerprint == S6_HISTORICAL_FINGERPRINT
...
def test_schema_rejects_spin_8(tmp_path):
    raw = _raw_manifest()
    raw["cases"][0]["spin"] = 8
    path = tmp_path / "manifest.json"
    path.write_text(json.dumps(raw))
...
def test_schema_rejects_wrong_level3_s6_reference_identity(tmp_path):
    raw = _raw_manifest()
    raw["level3_s6_reference"]["manifest_fingerprint"] = "f" * 64
    path = tmp_path / "manifest.json"
    path.write_text(json.dumps(raw))
    with pytest.raises(ValueError):
        load_manifest(path)
...
def test_level3_s7_manifest_case_rejects_out_of_contract_geometry():
    with pytest.raises(ValueError):
        Level3S7ManifestCase(geometry="hexagon", spin=7)


def test_level3_s7_manifest_case_rejects_spin_other_than_7():
    with pytest.raises(ValueError):
        Level3S7ManifestCase(geometry="triangle", spin=8)
    with pytest.raises(ValueError):
        Level3S7ManifestCase(geometry="triangle", spin=6)
...
def test_level3_s6_reference_identity_rejects_wrong_repository_commit():
    with pytest.raises(ValueError):
        Level3S6ReferenceIdentity(
            campaign_id="level3-s6-truncation-extension-v1",
            manifest_fingerprint=S6_HISTORICAL_FINGERPRINT,
            repository_commit="0" * 40,
