"""Unit tests for cosmobox.level3.frozen_s6_reference (lot L3-AC).

Exercises the real, versioned results/level3/level3-s6-truncation-extension-v1/
artifacts (frozen by lot L3-Z) for the positive paths, and tmp_path
...
from cosmobox.level3.frozen_s6_reference import (
    LEVEL3_S6_REFERENCE_DIMENSIONS,
    FrozenS6ReferenceIntegrityError,
    frozen_metric_comparison,
    frozen_s6_reference_root,
    load_campaign_summary,
    load_case_result,
    reconstruct_case_execution_result,
    verify_frozen_s6_reference,
    verify_sha256sums,
)

REAL_ROOT = frozen_s6_reference_root()


@pytest.fixture()
def tampered_root(tmp_path: Path) -> Path:
    """A full copy of the real, versioned frozen S6 reference directory,
    so tests can corrupt bytes/provenance without ever touching the real
    tracked artifacts."""
    copy_root = tmp_path / "level3-s6-truncation-extension-v1"
...
# ---------------------------------------------------------------------------
# Reconstruction: frozen S6 case-result JSON -> CaseExecutionResult, never run_case
# ---------------------------------------------------------------------------


def test_reconstruct_case_execution_result_from_real_triangle_s6():
...
def test_reconstructed_s6_feeds_compare_spin_pair_without_modification():
    # Synthetic S7 side, matching the pattern already established for
    # frozen-S5-vs-real-S6 comparisons in tests/level3/test_level3_s6_serialization.py.
    ...
    result_s7 = CaseExecutionResult(
        spec=CaseSpec(geometry="triangle", spin=7), dimension=n, group_count=n, entries=entries,
...
def test_frozen_metric_comparison_carries_s2_s3_s4_s5_s6_data():
    entry = frozen_metric_comparison("triangle", "M_TT")
    assert {
        "delta_hl_s2", "delta_hl_s3", "delta_hl_s4", "delta_hl_s5", "delta_hl_s6",
        "direction_s2", "direction_s3", "direction_s4", "direction_s5", "direction_s6",
        "t_x_45", "t_x_56", "c_x_45", "d_x_45", "c_x_56", "d_x_56",
    }.issubset(entry)
...
def test_verify_sha256sums_rejects_missing_listed_artifact(tampered_root):
    (tampered_root / "cases" / "ring5-S6.json").unlink()
    with pytest.raises(FrozenS6ReferenceIntegrityError):
        verify_sha256sums(root=tampered_root)
