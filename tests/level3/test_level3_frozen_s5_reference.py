"""Unit tests for cosmobox.level3.frozen_s5_reference (lot L3-W).

Exercises the real, versioned results/level3/level3-s5-truncation-extension-v1/
artifacts (frozen by lot L3-S) for the positive paths, and tmp_path
...
from cosmobox.level3.frozen_s5_reference import (
    LEVEL3_S5_REFERENCE_DIMENSIONS,
    FrozenS5ReferenceIntegrityError,
    frozen_metric_comparison,
    frozen_s5_reference_root,
    load_campaign_summary,
    load_case_result,
    reconstruct_case_execution_result,
    verify_frozen_s5_reference,
    verify_sha256sums,
)

REAL_ROOT = frozen_s5_reference_root()


@pytest.fixture()
def tampered_root(tmp_path: Path) -> Path:
    """A full copy of the real, versioned frozen S5 reference directory,
    so tests can corrupt bytes/provenance without ever touching the real
    tracked artifacts."""
    copy_root = tmp_path / "level3-s5-truncation-extension-v1"
...
# ---------------------------------------------------------------------------
# Reconstruction: frozen S5 case-result JSON -> CaseExecutionResult, never run_case
# ---------------------------------------------------------------------------


def test_reconstruct_case_execution_result_from_real_triangle_s5():
    ...
    assert result.spec.spin == 5
    assert result.dimension == 208
...
def test_reconstructed_s5_feeds_compare_spin_pair_without_modification():
    # Synthetic S6 side, matching the pattern already established for
    # frozen-S4-vs-real-S5 comparisons in tests/level3/test_level3_s5_serialization.py.
    ...
    result_s6 = CaseExecutionResult(
        spec=CaseSpec(geometry="triangle", spin=6), dimension=n, group_count=n, entries=entries,
...
def test_frozen_metric_comparison_carries_s2_s3_s4_s5_data():
    ...
    assert {
        ...
        "t_x_34", "t_x_45", "c_x_34", "d_x_34", "c_x_45", "d_x_45",
    }.issubset(entry)
...
def test_verify_sha256sums_rejects_missing_listed_artifact(tampered_root):
    (tampered_root / "cases" / "ring5-S5.json").unlink()
    with pytest.raises(FrozenS5ReferenceIntegrityError):
        verify_sha256sums(root=tampered_root)
