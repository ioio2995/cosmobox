"""Machine-readable Level3 S7 campaign manifest (lot L3-AC).

experiments/level3/preregistered-s7-manifest-v1.json is the normative
machine-readable transcription of the Level3 S7 fourth-extension
scientific contract (docs/levels/level3/s7-fourth-extension-preregistration.md,
frozen at commit cc0ffdadbeb7d40fc443ab11150c9f47e3e12776). This module
performs no scientific computation: it loads, validates (Draft 2020-12,
schemas/level3/s7-campaign-manifest-v1.schema.json), computes
...
the S4/S5/S6 manifests; no second algorithm is invented here), and
exposes typed, frozen views.

Deliberately independent of experiments.level2.manifest.Level2Manifest,
experiments.level3.manifest.Level3Manifest (the frozen S4 contract),
experiments.level3.s5_manifest.Level3S5Manifest (the frozen S5 contract),
AND experiments.level3.s6_manifest.Level3S6Manifest (the frozen S6
contract): this manifest pins the frozen Level2 (S2/S3), frozen Level3
S4, frozen Level3 S5, AND frozen Level3 S6 reference identities as
explicit, literal data (level2_reference, level3_s4_reference,
level3_s5_reference, level3_s6_reference), never by importing or loading
any of those manifest modules at runtime. None of experiments/level3/
manifest.py, experiments/level3/s5_manifest.py, or
experiments/level3/s6_manifest.py is imported, modified, or touched by
this file's existence -- their own fingerprint mechanisms, constants, and
frozen S4/S5/S6 contracts are entirely unaffected (verified by dedicated
regression tests).
"""
...
_MANIFEST_PATH = Path(__file__).resolve().parent / "preregistered-s7-manifest-v1.json"
_SCHEMA_PATH = Path(__file__).resolve().parents[2] / "schemas" / "level3" / "s7-campaign-manifest-v1.schema.json"

GEOMETRIES = ("triangle", "ring4", "ring5")
SPIN = 7
"""The only spin this manifest's cases may declare -- Level3's fourth
extension, per docs/levels/level3/s7-fourth-extension-preregistration.md.
...
"""triangle, ring4, ring5, all at S=7 -- geometry-major order, mirroring
the S4/S5/S6 manifests' own convention. Never S2/S3/S4/S5/S6 (those are
the frozen references, never a case this manifest executes) and never S8
(a possible future, separately pre-registered extension)."""

EXPECTED_S7_DIMENSIONS: dict[str, int] = {"triangle": 288, "ring4": 992, "ring5": 3520}
"""Hilbert-space dimensions established by the L3-AA capability preflight
...
class Level3S7ManifestCase:
    ...
    def __post_init__(self) -> None:
        ...
        if self.spin != SPIN:
            raise ValueError(f"spin must be exactly {SPIN} (the only Level3 S7 campaign case), got {self.spin!r}")


@dataclass(frozen=True, slots=True)
class NumericalGuardReference:
    """The frozen L2-C1 reproducibility guards, transcribed -- never
    recomputed, never widened. Identical values to Level2's, S4's, S5's,
    and S6's own frozen guards: the S6<->S7 taxonomy reuses exactly the
    same numerical zero-resolution policy, per the S7 preregistration."""
...
class PhysicalParameters:
    ...
    (identical to Level2, S4, S5, and S6, per PHYSICAL_PARAMETERS =
    IDENTICAL_TO_LEVEL2_S4_S5_S6 in the S7 preregistration). Not consumed
...
class Level2ReferenceIdentity:
    """The frozen Level2 campaign identity Level3 S7 pins as its exclusive
    S=2/S=3 source -- literal, hard-checked values, identical to the
    S4/S5/S6 manifests' own pinned Level2 identity, never re-derived at
    runtime."""
...
class Level3S4ReferenceIdentity:
    """The frozen Level3 S4 campaign identity Level3 S7 pins as its
...
@dataclass(frozen=True, slots=True)
class Level3S5ReferenceIdentity:
    """The frozen Level3 S5 campaign identity Level3 S7 pins as its
    exclusive S=5 source -- literal, hard-checked values."""
    ...
    def __post_init__(self) -> None:
        if self.campaign_id != "level3-s5-truncation-extension-v1":
            raise ValueError(
                f"level3_s5_reference.campaign_id must be 'level3-s5-truncation-extension-v1', got {self.campaign_id!r}"
            )
        if self.manifest_fingerprint != "3160a8e6e0f9ae4a21c027a865e4d56527303ad644e3f45137baa2120f0c5d04":
            raise ValueError(
                "level3_s5_reference.manifest_fingerprint must be "
                f"'3160a8e6e0f9ae4a21c027a865e4d56527303ad644e3f45137baa2120f0c5d04', got {self.manifest_fingerprint!r}"
            )
        if self.repository_commit != "782f29eb9dcbc21ca2168e109997b0f59a12402f":
            raise ValueError(
                f"level3_s5_reference.repository_commit must be '782f29eb9dcbc21ca2168e109997b0f59a12402f', "
                f"got {self.repository_commit!r}"
            )
        if self.frozen_preregistration_commit != "496ba9484a6d7df9beb738607a5ffe23a75219ad":
            raise ValueError(
                "level3_s5_reference.frozen_preregistration_commit must be "
                f"'496ba9484a6d7df9beb738607a5ffe23a75219ad', got {self.frozen_preregistration_commit!r}"
            )


@dataclass(frozen=True, slots=True)
class Level3S6ReferenceIdentity:
    """The frozen Level3 S6 campaign identity Level3 S7 pins as its
    exclusive S=6 source -- literal, hard-checked values."""
    ...
    def __post_init__(self) -> None:
        if self.campaign_id != "level3-s6-truncation-extension-v1":
            raise ValueError(
                f"level3_s6_reference.campaign_id must be 'level3-s6-truncation-extension-v1', got {self.campaign_id!r}"
            )
        if self.manifest_fingerprint != "2e0f06b69139abeb0b225a3d2b715242d6a0c26d2d1067b789f26b4829f847ca":
            raise ValueError(
                "level3_s6_reference.manifest_fingerprint must be "
                f"'2e0f06b69139abeb0b225a3d2b715242d6a0c26d2d1067b789f26b4829f847ca', got {self.manifest_fingerprint!r}"
            )
        if self.repository_commit != "55ea89be3a744953ff677754ff2ca22dbf116428":
            raise ValueError(
                f"level3_s6_reference.repository_commit must be '55ea89be3a744953ff677754ff2ca22dbf116428', "
                f"got {self.repository_commit!r}"
            )
        if self.frozen_preregistration_commit != "1c19a01e051c3df6f547afe0816a795989fdac3b":
            raise ValueError(
                "level3_s6_reference.frozen_preregistration_commit must be "
                f"'1c19a01e051c3df6f547afe0816a795989fdac3b', got {self.frozen_preregistration_commit!r}"
            )


@dataclass(frozen=True, slots=True)
class Level3S7Manifest:
    manifest_version: str
    campaign_id: str
    schema_version: str
    branch: str
    frozen_preregistration_commit: str
    cases: tuple[Level3S7ManifestCase, ...]
    ...
    level3_s5_reference: Level3S5ReferenceIdentity
    level3_s6_reference: Level3S6ReferenceIdentity
    expected_s7_dimensions: dict[str, int]
    ...
    def __post_init__(self) -> None:
        if self.manifest_version != "level3-s7-reference-v1":
            raise ValueError(f"manifest_version must be 'level3-s7-reference-v1', got {self.manifest_version!r}")
        ...
        if commit != "cc0ffdadbeb7d40fc443ab11150c9f47e3e12776":
            raise ValueError(
                f"frozen_preregistration_commit must be the frozen S7 preregistration commit "
                f"'cc0ffdadbeb7d40fc443ab11150c9f47e3e12776', got {commit!r}"
            )
        ...
        if not self.full_spectrum_required:
            raise ValueError("full_spectrum_required must be true for the Level3 S7 campaign")
        ...
        if not isinstance(self.level3_s5_reference, Level3S5ReferenceIdentity):
            raise ValueError(
                f"level3_s5_reference must be a Level3S5ReferenceIdentity, got {type(self.level3_s5_reference)}"
            )
        if not isinstance(self.level3_s6_reference, Level3S6ReferenceIdentity):
            raise ValueError(
                f"level3_s6_reference must be a Level3S6ReferenceIdentity, got {type(self.level3_s6_reference)}"
            )
        ...
        if actual_order != _EXPECTED_CASE_ORDER:
            raise ValueError(
                f"cases must be exactly the three frozen Level3 S7 combinations in geometry-major "
                f"order {_EXPECTED_CASE_ORDER}, got {actual_order}"
            )

        if dict(self.expected_s7_dimensions) != EXPECTED_S7_DIMENSIONS:
            raise ValueError(
                f"expected_s7_dimensions must be exactly {EXPECTED_S7_DIMENSIONS}, got {dict(self.expected_s7_dimensions)}"
            )
        ...
        if "S8" not in self.forbidden_extensions:
            raise ValueError(f"forbidden_extensions must include 'S8', got {self.forbidden_extensions!r}")
...
def parse_manifest(raw: dict) -> Level3S7Manifest:
    ...
    level3_s5_reference = raw["level3_s5_reference"]
    level3_s6_reference = raw["level3_s6_reference"]
    return Level3S7Manifest(
        manifest_version=raw["manifest_version"],
        campaign_id=raw["campaign_id"],
        schema_version=raw["schema_version"],
        branch=raw["branch"],
        frozen_preregistration_commit=raw["frozen_preregistration_commit"],
        cases=tuple(Level3S7ManifestCase(geometry=entry["geometry"], spin=entry["spin"]) for entry in raw["cases"]),
        physical_parameters=PhysicalParameters(
        ...
        level3_s5_reference=Level3S5ReferenceIdentity(
            campaign_id=level3_s5_reference["campaign_id"],
            manifest_fingerprint=level3_s5_reference["manifest_fingerprint"],
            repository_commit=level3_s5_reference["repository_commit"],
            frozen_preregistration_commit=level3_s5_reference["frozen_preregistration_commit"],
        ),
        level3_s6_reference=Level3S6ReferenceIdentity(
            campaign_id=level3_s6_reference["campaign_id"],
            manifest_fingerprint=level3_s6_reference["manifest_fingerprint"],
            repository_commit=level3_s6_reference["repository_commit"],
            frozen_preregistration_commit=level3_s6_reference["frozen_preregistration_commit"],
        ),
        expected_s7_dimensions=dict(raw["expected_s7_dimensions"]),
...
def load_manifest(path: Path = _MANIFEST_PATH) -> Level3S7Manifest:
