"""Machine-readable Level3 S6 campaign manifest (lot L3-W).

experiments/level3/preregistered-s6-manifest-v1.json is the normative
machine-readable transcription of the Level3 S6 third-extension
scientific contract (docs/levels/level3/s6-third-extension-preregistration.md,
frozen at commit 1c19a01e051c3df6f547afe0816a795989fdac3b). This module
performs no scientific computation: it loads, validates (Draft 2020-12,
schemas/level3/s6-campaign-manifest-v1.schema.json), computes
...
the S4/S5 manifests; no second algorithm is invented here), and exposes
typed, frozen views.

Deliberately independent of experiments.level2.manifest.Level2Manifest,
experiments.level3.manifest.Level3Manifest (the frozen S4 contract), AND
experiments.level3.s5_manifest.Level3S5Manifest (the frozen S5 contract):
this manifest pins the frozen Level2 (S2/S3), frozen Level3 S4, AND frozen
Level3 S5 reference identities as explicit, literal data (level2_reference,
level3_s4_reference, level3_s5_reference), never by importing or loading
any of those manifest modules at runtime. Neither experiments/level3/
manifest.py nor experiments/level3/s5_manifest.py is imported, modified,
or touched by this file's existence -- their own fingerprint mechanisms,
constants, and frozen S4/S5 contracts are entirely unaffected (verified by
...
_MANIFEST_PATH = Path(__file__).resolve().parent / "preregistered-s6-manifest-v1.json"
_SCHEMA_PATH = Path(__file__).resolve().parents[2] / "schemas" / "level3" / "s6-campaign-manifest-v1.schema.json"

GEOMETRIES = ("triangle", "ring4", "ring5")
SPIN = 6
"""The only spin this manifest's cases may declare -- Level3's third
extension, per docs/levels/level3/s6-third-extension-preregistration.md.
...
"""triangle, ring4, ring5, all at S=6 -- geometry-major order, mirroring
the S4/S5 manifests' own convention. Never S2/S3/S4/S5 (those are the
frozen references, never a case this manifest executes) and never S7 (a
possible future, separately pre-registered extension)."""

EXPECTED_S6_DIMENSIONS: dict[str, int] = {"triangle": 248, "ring4": 852, "ring5": 3016}
"""Hilbert-space dimensions established by the L3-U capability preflight
...
class Level3S6ManifestCase:
    ...
    def __post_init__(self) -> None:
        ...
        if self.spin != SPIN:
            raise ValueError(f"spin must be exactly {SPIN} (the only Level3 S6 campaign case), got {self.spin!r}")


@dataclass(frozen=True, slots=True)
class NumericalGuardReference:
    """The frozen L2-C1 reproducibility guards, transcribed -- never
    recomputed, never widened. Identical values to Level2's, S4's, and
    S5's own frozen guards: the S5<->S6 taxonomy reuses exactly the same
    numerical zero-resolution policy, per the L3-T preregistration."""
...
class PhysicalParameters:
    ...
    (identical to Level2, S4, and S5, per PHYSICAL_PARAMETERS =
    IDENTICAL_TO_LEVEL2_S4_S5 in the L3-T preregistration). Not consumed
...
class Level2ReferenceIdentity:
    """The frozen Level2 campaign identity Level3 S6 pins as its exclusive
    S=2/S=3 source -- literal, hard-checked values, identical to the
    S4/S5 manifests' own pinned Level2 identity, never re-derived at
    runtime."""
...
class Level3S4ReferenceIdentity:
    """The frozen Level3 S4 campaign identity Level3 S6 pins as its
...
class Level3S5ReferenceIdentity:
    """The frozen Level3 S5 campaign identity Level3 S6 pins as its
...
@dataclass(frozen=True, slots=True)
class Level3S6Manifest:
    manifest_version: str
    campaign_id: str
    schema_version: str
    branch: str
    frozen_preregistration_commit: str
    cases: tuple[Level3S6ManifestCase, ...]
    ...
    expected_s6_dimensions: dict[str, int]
    ...
    def __post_init__(self) -> None:
        if self.manifest_version != "level3-s6-reference-v1":
            raise ValueError(f"manifest_version must be 'level3-s6-reference-v1', got {self.manifest_version!r}")
        ...
        if commit != "1c19a01e051c3df6f547afe0816a795989fdac3b":
            raise ValueError(
                f"frozen_preregistration_commit must be the frozen L3-T commit "
                f"'1c19a01e051c3df6f547afe0816a795989fdac3b', got {commit!r}"
            )
        ...
        if not self.full_spectrum_required:
            raise ValueError("full_spectrum_required must be true for the Level3 S6 campaign")
        ...
        if actual_order != _EXPECTED_CASE_ORDER:
            raise ValueError(
                f"cases must be exactly the three frozen Level3 S6 combinations in geometry-major "
                f"order {_EXPECTED_CASE_ORDER}, got {actual_order}"
            )

        if dict(self.expected_s6_dimensions) != EXPECTED_S6_DIMENSIONS:
            raise ValueError(
                f"expected_s6_dimensions must be exactly {EXPECTED_S6_DIMENSIONS}, got {dict(self.expected_s6_dimensions)}"
            )
        ...
        if "S7" not in self.forbidden_extensions:
            raise ValueError(f"forbidden_extensions must include 'S7', got {self.forbidden_extensions!r}")
...
def parse_manifest(raw: dict) -> Level3S6Manifest:
    ...
    return Level3S6Manifest(
        manifest_version=raw["manifest_version"],
        campaign_id=raw["campaign_id"],
        schema_version=raw["schema_version"],
        branch=raw["branch"],
        frozen_preregistration_commit=raw["frozen_preregistration_commit"],
        cases=tuple(Level3S6ManifestCase(geometry=entry["geometry"], spin=entry["spin"]) for entry in raw["cases"]),
        physical_parameters=PhysicalParameters(
        ...
        expected_s6_dimensions=dict(raw["expected_s6_dimensions"]),
...
def load_manifest(path: Path = _MANIFEST_PATH) -> Level3S6Manifest:
