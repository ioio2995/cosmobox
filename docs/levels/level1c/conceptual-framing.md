...
- dépendance à la saveur : ces quatre grandeurs sont dérivées de `G^{alpha,beta}[P]`, une matrice 2×2 dans l'espace des indices de saveur `(alpha, beta)` — deux propriétés distinctes s'y appliquent, à ne jamais confondre : (1) `G` lui-même hérite de l'invariance de la prescription canonique sous changement de base interne du multiplet spectral (`Psi -> Psi V`, spécification §4) — ceci ne relève pas de la saveur, c'est la même invariance de base qui justifie `rho_M = Pi_M / d` pour tout opérateur restreint, `G` y compris ; (2) séparément, ces quatre grandeurs sont elles-mêmes construites pour être invariantes/covariantes sous rotation de saveur SU(2), une transformation distincte agissant sur les indices `(alpha, beta)` de `G` — c'est cette seconde propriété qui leur permet d'éliminer la dépendance de saveur plutôt que de la porter ;
