...
- **V23 exige une couverture structurelle complète avant tout verdict** : `N` diagonales (une par site) et exactement `N(N-1)` paires ordonnées hors-diagonale (les deux ordres, aucune paire diagonale parmi elles, aucun site étranger) — un corpus incomplet ou incohérent lève une erreur structurelle, jamais un verdict `is_valid=False`/`True` ; seul un corpus structurellement complet dont la somme physique manque `T(T+1)` au-delà de la tolérance produit `is_valid=False`.
