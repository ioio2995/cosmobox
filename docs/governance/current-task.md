...
```text
e4cc72598f0b334a41220d1873f539cd0210c8f6
```
...
- `7ef4669b92f1986a507313c5ef5a62e6dddfe839` — livraison candidate 1B-9d (extraction/comparaison des observables inter-S), conforme sur l'extraction/comparaison elle-même mais acceptation initialement suspendue pour deux défauts de contrat strictement bornés (voir « Lot actif » ci-dessous).

**Le pointeur ci-dessus (`e4cc72598f0b334a41220d1873f539cd0210c8f6`) reste le dernier commit formellement accepté** : ni la livraison candidate 1B-9d, ni son correctif, ne sont encore acceptés — même règle que celle déjà appliquée pendant la suspension de 1B-9b et de 1B-9c.

1B-9a — conception de la couche d'analyse inter-S sur artefacts normatifs — **conception acceptée, y compris son addendum**, aucun code livré par ce lot (design uniquement). Décisions gelées pour l'implémentation, reprises et mises en œuvre par 1B-9c/1B-9d ci-dessous :
...
1B-9d — extraction et comparaison des observables inter-S.

**Ce lot ne calcule aucun verdict de robustesse.** Interdit : appel à `evaluate_robustness` ; production de `robust`/`non_robust`/`indeterminate`/`threshold` ; recodage de `max(0.05, 0.15 * amplitude)`. Le lot suivant (1B-9e, non démarré) décidera comment appliquer `evaluate_robustness` aux comparaisons construites ici.

**Historique de ce sous-lot** : livraison candidate `7ef4669b92f1986a507313c5ef5a62e6dddfe839` — conforme sur l'extraction/comparaison elle-même, mais acceptation initialement suspendue pour deux défauts de contrat strictement bornés : (1) `flavor_singular_value_ratio` ne vérifiait que l'égalité high/low de `normalization`, sans exiger `== "raw_G"` — un artefact forgé `None`/`None` (ou toute autre chaîne identique invalide) des deux côtés aurait été accepté comme comparable ; (2) `InterSObservableComparisonReport.comparisons` ne rejetait pas réellement une `list` (mutable sous `frozen=True`) ni un élément d'un type inattendu, et `ScalarObservableComparison` ne vérifiait pas le type/la finitude numérique réelle des valeurs non-null (`str`/`bool`/`NaN`/`±Inf` auraient été acceptés) → **correctif décrit ci-dessous, livré au commit indiqué dans « Dernier commit accepté »**.

Objectif exact : à partir du `CampaignArtifactIndex` + `InterSMatchingReport` acceptés, sélectionne les documents high/low réellement comparables pour la liste FERMÉE `O_ij_raw`/`rho_QQ`/`C_TT_conn`/`flavor_singular_value_ratio`, calcule `gamma_O` (uniquement pour `O_ij_raw`, via `cosmobox.level1.robustness.compute_gamma_o` — inchangée, jamais recodée), et expose les paires numériques brutes des trois autres. Ne traite pas `G_occ`/`path_phase_coherence`/`flavor_singlet`/`raw_G`/`flavor_singular_values`/diagnostics/labels/orbit statistics.

**Écart au plan constaté et corrigé sans blocage (fait d'ingénierie vérifiable sur le code/les artefacts déjà acceptés, pas une hypothèse scientifique)** : le mandat décrivait le payload de `C_TT_conn` comme un `NormalizedMoment` sérialisé (`{"value":.., "null_reason":..}`). Vérification faite sur `results.py` (table close `RAW_OBSERVABLE_KINDS`, qui inclut `C_TT_conn`) et sur les documents réels : `C_TT_conn` est en réalité un `record_kind="raw_observable"` à payload `float` nu, toujours présent, sans notion de `null_reason` — exactement comme `C_QQ_raw`/`C_TT_raw`. Traité en conséquence (`ScalarObservableComparison.high_null_reason`/`low_null_reason` valent toujours `None` pour `C_TT_conn`, jamais fabriqués).

**Désambiguïsation `record_kind` pour `O_ij_raw`** (fait constaté sur les artefacts réels) : `observable_kind == "O_ij_raw"` est produit sous TROIS `record_kind` différents partageant le même `identity.path`/`flavor_component` — `raw_observable` (lecture brute complexe par chemin, celle comparée ici), `orbit_statistic` (agrégat d'orbite, `OrbitResultPayload`), `restricted_diagnostic` (`NonHermitianRestrictedDiagnostics`). La clé de comparabilité `(path, flavor_component, normalization)` seule serait donc ambiguë pour `O_ij_raw` : `record_kind` est toujours fixé en plus (dérivé des tables closes `RAW_OBSERVABLE_KINDS`/`NORMALIZED_OBSERVABLE_KINDS`/`FLAVOR_DIAGNOSTIC_KINDS` de `results.py`, jamais une table privée dupliquée) avant toute recherche de correspondance.

**Comparabilité (1B-9a, gelée)** : `identity.path` (tuple complet, jamais réduit à `(i,j)`/aux extrémités), `identity.flavor_component`, `identity.normalization` — tous les trois exactement égaux, dans le contexte du groupe spectral déjà apparié. Jamais `spectral_window_group_index`/`representative_energy`/`target_id`/`spin`. `flavor_singular_value_ratio` est explicitement path-dependent : deux chemins de mêmes extrémités mais de contenu différent ne sont jamais comparables.

**`flavor_singular_value_ratio` — exigence `normalization == "raw_G"` (correctif)** : l'égalité high/low de `normalization` (déjà garantie par la clé de comparabilité partagée) n'est PAS suffisante. Une fois une paire trouvée par clé, `_require_valid_flavor_ratio_normalization` exige en plus que `identity.normalization` vaille exactement `"raw_G"` (`FLAVOR_SINGULAR_VALUE_RATIO_NORMALIZATION`) des DEUX côtés — un artefact `None`/`None` ou toute autre chaîne identique invalide est rejeté comme incohérence structurelle (`InterSObservableComparisonError`), jamais silencieusement accepté ni corrigé vers la constante attendue. Les autres observables ne portent aucune contrainte de constante supplémentaire au-delà de leurs axes déjà gelés.

**Cardinalité** : un document low dupliqué à la même clé → `InterSObservableComparisonError` ; un document high éligible sans exactement un low correspondant → `InterSObservableComparisonError` (le corpus normatif est supposé intégralement analysable) ; un document low sans high correspondant n'est pas une erreur, simplement jamais visité (le parcours se fait uniquement sur `high_group.documents`).

**Null** : `value is None ⟺ null_reason` défini, conservé indépendamment côté high et côté low, jamais remplacé par `0`/un epsilon. `O_ij_raw` n'a pas de notion de null (payload complexe toujours présent) ; `gamma_o` lui-même n'est jamais `None` pour un `GammaOComparison` (bien que sa propre valeur interne puisse l'être, via le floor de `compute_gamma_o`).

**API** : `GammaOComparison` (`O_ij_raw` uniquement : `high_value`/`low_value` complexes toujours présents ET finis, `gamma_o: NormalizedMoment` jamais `None`) et `ScalarObservableComparison` (les 3 autres : `high_value`/`low_value: float | None`, `high_null_reason`/`low_null_reason`, **aucun champ `gamma_o`** — état invalide rendu irreprésentable par le typage plutôt que vérifié à l'exécution) — deux dataclasses spécialisées plutôt qu'une seule générique, comme permis par le mandat. `InterSObservableComparisonReport(campaign_id, manifest_fingerprint, repository_commit, comparisons)` — provenance copiée de la campagne SOURCE (`InterSMatchingReport`), jamais le commit du code d'analyse. Seuls les matches `exact_label_match` produisent des comparaisons ; un match non exact est silencieusement exclu de `comparisons`, jamais transformé en verdict.

**Durcissement des constructeurs publics (correctif)** : `InterSObservableComparisonReport.__post_init__` exige désormais réellement que `comparisons` soit un `tuple` (une `list` fournie par un appelant reste mutable sous `frozen=True` et n'est jamais convertie silencieusement) et que chaque élément soit un `GammaOComparison`/`ScalarObservableComparison`. `ScalarObservableComparison.__post_init__` exige désormais qu'une valeur non-null soit exactement un `float` fini (rejette `str`, `bool` — jamais confondu avec un float —, `NaN`, `±Inf`) et qu'un `null_reason` défini soit une `str` réellement non vide ; pour `observable_kind == "C_TT_conn"` spécifiquement, `high_value`/`low_value` ne doivent jamais être `None` et les deux `null_reason` doivent toujours être `None` (son payload natif est un `float` brut toujours présent). `GammaOComparison` reçoit la même exigence de finitude sur ses `high_value`/`low_value` complexes, par symétrie.

**Ordre** : celui de `InterSMatchingReport.matches`, puis — au sein d'un couple — l'ordre canonique exact de `high_group.documents` (assembly.py), `GammaOComparison`/`ScalarObservableComparison` entrelacés tels que rencontrés, jamais regroupés par type.

**Interdictions strictes de ce lot** : aucun appel à `evaluate_robustness`/`run_single_case`/`run_campaign`/`launch_normative_campaign`/`run_level1b_campaign` ; aucune modification de `src/cosmobox/level1/{matching,robustness,results,serialization}.py`, `scripts/level1b_campaign/*`, `experiments/level1/*`, `schemas/*`, `.github/workflows/*` ; aucun fichier écrit sous `/workspaces/level1b_campaign_output` ni ailleurs (`results/inter_s.json`, CSV, JSONL — aucune persistance dans ce lot) ; aucun `target_id` dans l'API.

Fichiers de ce lot : `scripts/level1b_analysis/comparisons.py` (`GammaOComparison`, `ScalarObservableComparison`, `InterSObservableComparisonReport`, `InterSObservableComparisonError`, `build_inter_s_observable_comparison_report`), `scripts/level1b_analysis/__init__.py` (docstring, ajustement mineur), `tests/scripts/level1b_analysis/test_comparisons.py`.

**Smoke-test réel mesuré** (`/workspaces/level1b_campaign_output`, lecture seule, non hardcodé, inchangé après correctif) : 712 comparaisons totales — `O_ij_raw` 416 paires (252 `gamma_O` null par floor), `rho_QQ` 96 paires (28 null high, 28 null low), `C_TT_conn` 96 paires (aucun null, par construction), `flavor_singular_value_ratio` 104 paires. Ceci est un relevé technique de valeurs, aucune conclusion de robustesse.

**Aucune fonctionnalité 1B-9e n'est autorisée** — ce lot ferme l'extraction/comparaison des observables, il n'ouvre aucun verdict. Acceptation encore suspendue, en attente de l'audit du présent correctif.
