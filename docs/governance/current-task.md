...
- `89fbfc27dc57469bcb9cbf818b9b6a4f405630ad` — livraison candidate 1B-9c (matching inter-S), conforme sur le matching lui-même mais **acceptation suspendue** : défaut principal (`low_window_truncated` incorrect sur le chemin sparse) et durcissement secondaire (sélection des deux plus grands spins non garantie distincte) — voir « Lot actif » ci-dessous pour le correctif appliqué par-dessus (livré ensuite, dans l'historique du dépôt).

**Le pointeur ci-dessus (`540b08096b7d67278a6f2eea8ce93dbf039887fe`) reste le dernier commit formellement accepté** : ni la livraison candidate 1B-9c, ni son correctif, ne sont encore acceptés — même règle que celle déjà appliquée pendant la suspension de 1B-9b.
...
**Historique de ce sous-lot** : livraison candidate `89fbfc27dc57469bcb9cbf818b9b6a4f405630ad` — conforme sur le matching proprement dit, mais acceptation suspendue pour un défaut principal (reconstruction générale incorrecte de `low_window_truncated` sur le chemin sparse) et un durcissement secondaire (sélection des deux plus grands spins non garantie distincte) → **correctif décrit ci-dessous, livré au commit indiqué dans « Dernier commit accepté »**.

**Construction des couples** : deux `CampaignCaseSpec` forment un couple admissible ssi même `geometry`, même `hamiltonian_identity_without_spin` (réutilise `indexing._case_hamiltonian_identity_tuple`, jamais dupliqué), même `sector_id`. Les cas sont regroupés par leur propre `spin` ; seules les deux plus grandes valeurs de `spin` DISTINCTES sont retenues comme `(S_high, S_low)` — jamais les deux premières entrées d'un tri par spin décroissant, qui pourrait apparier deux cas de même spin en cas d'égalité en tête. Une identité avec moins de deux spins distincts ne produit aucun couple (pas d'erreur). Sépare naturellement `reference`/`j_break` sans filtrage par nom : `j_break` change une valeur de `J`, donc `hamiltonian_identity_without_spin` diffère et le cas reste seul dans son identité. **Si plusieurs `CampaignCaseSpec` partagent la même identité physique ET le même spin retenu (`S_high` ou `S_low`)** — aucun invariant manifeste/planning connu de ce module ne garantit que cela ne puisse arriver — `_build_case_pairs` lève `InterSMatchingError` plutôt que de choisir arbitrairement (jamais observé dans la campagne actuelle).
...
**`low_window_truncated`** (corrigé) : reconstruit depuis `CampaignCaseSpec`/`SpectrumOptions` du cas low en reproduisant uniquement la logique de COMPTAGE déjà figée par `cosmobox.level0.reports._compute_spectrum` (`_reconstruct_computed_eigenvalue_count`), jamais une diagonalisation : `dimension == 0` et le cas non calculable selon les guardrails (`dimension > max_sparse_dimension` sans `force`) lèvent tous deux `InterSMatchingError` (Level0 ne construit jamais de `DegeneracyReport` dans ces deux cas — aucun groupe n'aurait pu être indexé) ; `dimension == 1` → exactement 1 eigenvalue calculée (chemin direct) ; dense (`dimension <= max_dense_dimension`) → `min(spectral_window, dimension)` ; sparse (`force` ou `dimension <= max_sparse_dimension`) → `min(spectral_window, dimension - 1)`. **L'ancienne formule (`spectral_window < physical_dimension`) était fausse sur le chemin sparse** : `eigsh` ne calcule jamais plus de `dimension - 1` eigenvalues, donc un cas sparse est toujours tronqué dès que `dimension >= 2`, indépendamment de `spectral_window` — l'ancienne formule renvoyait `False` à tort dès que `spectral_window >= physical_dimension`. Aucun nouveau champ, `ResultRecord` v2 et `serialization.py` inchangés. Pour les 3 cas low actuels de la campagne (`dimension <= max_dense_dimension = 2000`), le correctif ne change pas le résultat mesuré.
...
**Smoke-test réel mesuré** (`/workspaces/level1b_campaign_output`, lecture seule, non hardcodé, inchangé après correctif) : 3 couples, 7 groupes high analysés, 7/7 `exact_label_match` — `triangle`/`ring4`/`ring5` reference S3→S2, 2/2/3 groupes respectivement.

**Aucune fonctionnalité 1B-9d n'est autorisée** — ce lot ferme le matching inter-S structurel, il n'ouvre rien de nouveau (pas de robustesse, pas de gamma). Acceptation encore suspendue, en attente de l'audit du présent correctif.
