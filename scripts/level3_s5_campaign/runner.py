"""End-to-end Level3 S5 normative campaign runner (lot L3-P).

Wires together every L3-P building block (experiments.level3.s5_manifest,
scripts.level3_s5_campaign.provenance/planning/gates/outputs,
cosmobox.level3.s5_serialization, cosmobox.level3.execution.run_case/
compare_spin_pair, cosmobox.level3.frozen_reference (Level2),
cosmobox.level3.frozen_s4_reference (S4)) into one ordered, explicit
...
Order of operations (L3-P mandate):
  1. load manifest
  ...
  4. verify the frozen Level2 reference (SHA256SUMS + schema + pinned
     provenance, all-or-nothing, before any S5 execution)
  5. verify the frozen Level3 S4 reference (SHA256SUMS + schema + pinned
     provenance, all-or-nothing, before any S5 execution)
  6. persist the campaign manifest snapshot
  7. plan exactly three frozen CaseSpec (planning.plan_campaign)
  8. run execution.run_case(spec) per case
  9. validate_case_result(result)     (gates)
 10. serialize case-result            (s5_serialization.case_result_payload)
 11. atomically persist case artifact (outputs.write_case_result)
 12. retain in-memory result for later geometry comparison
 13. after 3/3 cases, reconstruct each geometry's frozen S4 reference and
     call execution.compare_spin_pair -- never before all three S5 cases
     are persisted
 14. validate_campaign_completeness (3/3 cases, 3/3 comparisons)  (gates)
 15. verify all persisted case artifacts share provenance         (outputs)
 16. build campaign-summary (s5_serialization.campaign_summary_payload,
     self-validating, embeds both pinned reference identities)
 17. atomically write campaign-summary.json                       (outputs)
...
S=5 case during lot L3-P (REAL_S5_EXECUTION = FORBIDDEN for this lot).
...
(tests/scripts/level3_s5_campaign/test_level3_s5_campaign_runner.py) --
no test in this lot calls run_campaign in a way that reaches a real
CaseSpec(..., spin=5) diagonalization.
"""
...
from cosmobox.level3 import frozen_reference, frozen_s4_reference, s5_serialization
from cosmobox.level3.execution import CaseExecutionResult, SpinPairComparison, compare_spin_pair, run_case
from experiments.level3.s5_manifest import Level3S5Manifest, load_manifest
...
def run_campaign(
    output_root: Path,
    *,
    manifest: Level3S5Manifest | None = None,
    repo_root: str | None = None,
) -> Path:
    """Run the full three-case Level3 S5 campaign and, once every
    ...
    # All-or-nothing verification of BOTH frozen references (SHA256SUMS,
    # schema, pinned provenance) BEFORE any S5 execution. No fallback, no
    ...
    frozen_s4_campaign_summary = frozen_s4_reference.load_campaign_summary()
    ...
    for spec in case_specs:
        ...
        document = s5_serialization.case_result_payload(
            result,
    ...
    # No normative comparison before all three S5 cases are persisted.
    geometry_comparisons: list[SpinPairComparison] = []
    for result in sorted(case_results, key=lambda r: r.spec.geometry):
        s4_reference = frozen_s4_reference.reconstruct_case_execution_result(result.spec.geometry)
        geometry_comparisons.append(compare_spin_pair(s4_reference, result))
    ...
    summary_document = s5_serialization.campaign_summary_payload(
        geometry_comparisons,
        frozen_s4_campaign_summary=frozen_s4_campaign_summary,
        campaign_id=campaign_provenance.campaign_id,
        manifest_fingerprint=campaign_provenance.manifest_fingerprint,
        repository=campaign_provenance.repository,
        repository_commit=campaign_provenance.repository_commit,
