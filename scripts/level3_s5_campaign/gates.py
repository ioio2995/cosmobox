"""Structural, protocol-only validation gates for Level3 S5 campaign
results.
...
runtime conditions, for the same reason scripts/level3_campaign/gates.py
(S4) does not re-check them: cosmobox.level3.execution.CaseExecutionResult
...
group_count has no pre-established reference: the L3-N capability
preflight only built the physical basis (dimension), never a Hamiltonian
or a spectrum -- so no group_count is known in advance for S=5. Only
dimension (established by L3-N) is cross-checked exactly; group_count is
...
from experiments.level3.s5_manifest import EXPECTED_S5_DIMENSIONS, SPIN


class CaseGateFailure(ValueError):
    """A case's structural/protocol invariants do not match the L3-N
...
def validate_case_result(result: CaseExecutionResult) -> None:
    """spin/geometry against the frozen Level3 S5 contract, dimension
    against EXPECTED_S5_DIMENSIONS (L3-N), and exact q coverage of [0,1]
    ...
    if result.spec.geometry not in EXPECTED_S5_DIMENSIONS:
        raise CaseGateFailure(
            f"geometry {result.spec.geometry!r} is not one of the three frozen Level3 S5 cases "
            f"{sorted(EXPECTED_S5_DIMENSIONS)}"
        )

    expected_dimension = EXPECTED_S5_DIMENSIONS[result.spec.geometry]
    if result.dimension != expected_dimension:
        raise CaseGateFailure(
            f"{result.spec.geometry}: dimension {result.dimension} does not match the L3-N reference "
...
def validate_campaign_completeness(
    ...
    """3/3 cases present (exactly the three frozen S5 combinations, no
    duplicate) and 3/3 geometry comparisons present (exactly the three
    frozen geometries, no duplicate)."""
    expected_geometries = set(EXPECTED_S5_DIMENSIONS)
    ...
    if actual_case_geometries != expected_geometries or any(spin != SPIN for _, spin in actual_cases):
        raise CampaignGateFailure(
            f"case_results does not cover exactly the three frozen Level3 S5 cases -- got {actual_cases}"
        )
