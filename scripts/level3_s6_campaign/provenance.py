"""Campaign-start Git provenance resolution for Level3 S6 campaigns.

Deliberately duplicated from scripts/level3_s5_campaign/provenance.py's
...
than imported, per the same isolation decision the S4/S5 campaign layers
themselves made from their predecessors.

CampaignProvenance additionally carries the frozen Level2 reference
identity, the frozen Level3 S4 reference identity, AND the frozen Level3
S5 reference identity, pinned to the exact constants
cosmobox.level3.frozen_reference, cosmobox.level3.frozen_s4_reference, and
cosmobox.level3.frozen_s5_reference themselves validate against -- so a
campaign's own provenance record and the three frozen-reference loaders
can never silently diverge on which campaigns are the S2/S3, S4, and S5
sources.
"""
...
"""The frozen repository identity every persisted Level3 S6 artifact must
...
class CampaignProvenance:
    """The provenance record every persisted Level3 S6 artifact must
    carry, resolved once per campaign, plus the pinned identity of all
    three frozen reference campaigns (Level2 for S=2/S=3, Level3 S4 for
    S=4, Level3 S5 for S=5)."""
...
def resolve_campaign_provenance(manifest: Level3S6Manifest, *, repo_root: str | None = None) -> CampaignProvenance:
    ...
    level2_reference_*/level3_s4_reference_*/level3_s5_reference_* come
    from `manifest` itself -- never re-derived. repository is always
    exactly REPOSITORY_IDENTITY."""
