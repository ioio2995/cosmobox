"""Atomic persistence for Level3 S6 campaign artifacts.

Deliberately duplicated from scripts/level3_s5_campaign/outputs.py's own
...
Campaign artifacts live under `results/level3/level3-s6-truncation-
...
def canonical_json_bytes(document: dict) -> bytes:
    ...
    scripts/level3_s5_campaign/outputs.py."""
...
class CaseArtifactIntegrityError(ValueError):
    ...
    provenance-consistency failure of a persisted Level3 S6 case-result
    artifact."""
...
def write_campaign_manifest(output_root: Path, campaign_id: str, raw_manifest: dict) -> Path:
    ...
    content (Level3S6Manifest.raw) that drove this campaign to
...
def load_and_verify_case_result(
    ...
    *,
    ...
    schema-valid against schemas/level3/s6-case-result-v1.schema.json
    (s6_serialization.validate_case_result_document), and verify its own
    ...
    try:
        s6_serialization.validate_case_result_document(document)
