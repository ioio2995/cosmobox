"""Atomic persistence for Level3 S4 campaign artifacts.

Deliberately duplicated from scripts/level2_campaign/outputs.py's own
...
no overwrite, no merge, no automatic deletion of an existing scientific
artifact. Per the same convention as Level2, campaign artifacts live
under `results/level3/...` (gitignored, exactly like the other
non-frozen entries under `results/` -- only the frozen Level2 reference
campaign under results/level2/level2-energy-regime-v1/ is versioned).
"""
...
def canonical_json_bytes(document: dict) -> bytes:
    ...
    scripts/level2_campaign/outputs.py."""
...
def atomic_write_json(path: Path, document: dict) -> None:
    ...
    on any failure -- a partially written file is never left where a
    reader could mistake it for `path`."""
    path = Path(path)
...
class CaseArtifactIntegrityError(ValueError):
    ...
    provenance-consistency failure of a persisted Level3 S4 case-result
    artifact."""
...
def write_campaign_summary(output_root: Path, campaign_id: str, document: dict) -> Path:
    ...
    is the campaign's COMPLETE marker: a campaign that fails or is
    interrupted before this call leaves no campaign-summary.json at all.
    Raises CampaignSummaryAlreadyExists (never silently overwriting) if
...
def write_campaign_manifest(output_root: Path, campaign_id: str, raw_manifest: dict) -> Path:
    ...
    content (Level3Manifest.raw) that drove this campaign to
...
def load_and_verify_case_result(
    ...
    *,
    ...
    schema-valid against schemas/level3/s4-case-result-v1.schema.json
    (serialization.validate_case_result_document), and verify its own
