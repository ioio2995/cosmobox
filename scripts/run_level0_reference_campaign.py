"""CLI entry point for the Level0 reference campaign (5B).

Run from the repository root as a module, so `scripts` resolves as a
package:

    python -m scripts.run_level0_reference_campaign --output-dir campaign_output

The importable pieces live in scripts/level0_reference_campaign/
(grid.py, runner.py, outputs.py); this file is only argument parsing and
the call into runner.run_campaign.
"""
...
from scripts.level0_reference_campaign.grid import build_reference_campaign_specs
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("campaign_output"), help="directory for runs/, manifest.json, summary.csv"
    )
    parser.add_argument(
