"""Campaign-start Git provenance resolution for Level3 S7 campaigns.

Deliberately duplicated from scripts/level3_s6_campaign/provenance.py's
own generic git primitives (resolve_code_commit, resolve_current_branch,
...
identity, the frozen Level3 S4 reference identity, the frozen Level3 S5
reference identity, AND the frozen Level3 S6 reference identity, pinned
to the exact constants cosmobox.level3.frozen_reference,
cosmobox.level3.frozen_s4_reference, cosmobox.level3.frozen_s5_reference,
and cosmobox.level3.frozen_s6_reference themselves validate against -- so
a campaign's own provenance record and the four frozen-reference loaders
can never silently diverge on which campaigns are the S2/S3, S4, S5, and
S6 sources.
"""
...
from cosmobox.level3.frozen_s5_reference import (
    LEVEL3_S5_REFERENCE_CAMPAIGN_ID,
    LEVEL3_S5_REFERENCE_FROZEN_PREREGISTRATION_COMMIT,
    LEVEL3_S5_REFERENCE_MANIFEST_FINGERPRINT,
    LEVEL3_S5_REFERENCE_REPOSITORY_COMMIT,
)
from cosmobox.level3.frozen_s6_reference import (
    LEVEL3_S6_REFERENCE_CAMPAIGN_ID,
    LEVEL3_S6_REFERENCE_FROZEN_PREREGISTRATION_COMMIT,
    LEVEL3_S6_REFERENCE_MANIFEST_FINGERPRINT,
    LEVEL3_S6_REFERENCE_REPOSITORY_COMMIT,
)
...
"""The frozen repository identity every persisted Level3 S7 artifact must
...
class CampaignProvenance:
    """The provenance record every persisted Level3 S7 artifact must
    carry, resolved once per campaign, plus the pinned identity of all
    four frozen reference campaigns (Level2 for S=2/S=3, Level3 S4 for
    S=4, Level3 S5 for S=5, Level3 S6 for S=6)."""
    ...
    level3_s5_reference_campaign_id: str
    level3_s5_reference_manifest_fingerprint: str
    level3_s5_reference_repository_commit: str
    level3_s5_reference_frozen_preregistration_commit: str
    level3_s6_reference_campaign_id: str
    level3_s6_reference_manifest_fingerprint: str
    level3_s6_reference_repository_commit: str
    level3_s6_reference_frozen_preregistration_commit: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.level3_s5_reference_campaign_id != LEVEL3_S5_REFERENCE_CAMPAIGN_ID:
            raise ValueError(
                f"level3_s5_reference_campaign_id must be {LEVEL3_S5_REFERENCE_CAMPAIGN_ID!r}, "
                f"got {self.level3_s5_reference_campaign_id!r}"
            )
        if self.level3_s5_reference_manifest_fingerprint != LEVEL3_S5_REFERENCE_MANIFEST_FINGERPRINT:
            raise ValueError(
                f"level3_s5_reference_manifest_fingerprint must be {LEVEL3_S5_REFERENCE_MANIFEST_FINGERPRINT!r}, "
                f"got {self.level3_s5_reference_manifest_fingerprint!r}"
            )
        if self.level3_s5_reference_repository_commit != LEVEL3_S5_REFERENCE_REPOSITORY_COMMIT:
            raise ValueError(
                f"level3_s5_reference_repository_commit must be {LEVEL3_S5_REFERENCE_REPOSITORY_COMMIT!r}, "
                f"got {self.level3_s5_reference_repository_commit!r}"
            )
        if self.level3_s5_reference_frozen_preregistration_commit != LEVEL3_S5_REFERENCE_FROZEN_PREREGISTRATION_COMMIT:
            raise ValueError(
                "level3_s5_reference_frozen_preregistration_commit must be "
                f"{LEVEL3_S5_REFERENCE_FROZEN_PREREGISTRATION_COMMIT!r}, "
                f"got {self.level3_s5_reference_frozen_preregistration_commit!r}"
            )
        if self.level3_s6_reference_campaign_id != LEVEL3_S6_REFERENCE_CAMPAIGN_ID:
            raise ValueError(
                f"level3_s6_reference_campaign_id must be {LEVEL3_S6_REFERENCE_CAMPAIGN_ID!r}, "
                f"got {self.level3_s6_reference_campaign_id!r}"
            )
        if self.level3_s6_reference_manifest_fingerprint != LEVEL3_S6_REFERENCE_MANIFEST_FINGERPRINT:
            raise ValueError(
                f"level3_s6_reference_manifest_fingerprint must be {LEVEL3_S6_REFERENCE_MANIFEST_FINGERPRINT!r}, "
                f"got {self.level3_s6_reference_manifest_fingerprint!r}"
            )
        if self.level3_s6_reference_repository_commit != LEVEL3_S6_REFERENCE_REPOSITORY_COMMIT:
            raise ValueError(
                f"level3_s6_reference_repository_commit must be {LEVEL3_S6_REFERENCE_REPOSITORY_COMMIT!r}, "
                f"got {self.level3_s6_reference_repository_commit!r}"
            )
        if self.level3_s6_reference_frozen_preregistration_commit != LEVEL3_S6_REFERENCE_FROZEN_PREREGISTRATION_COMMIT:
            raise ValueError(
                "level3_s6_reference_frozen_preregistration_commit must be "
                f"{LEVEL3_S6_REFERENCE_FROZEN_PREREGISTRATION_COMMIT!r}, "
                f"got {self.level3_s6_reference_frozen_preregistration_commit!r}"
            )


def resolve_campaign_provenance(manifest: Level3S7Manifest, *, repo_root: str | None = None) -> CampaignProvenance:
    ...
    level2_reference_*/level3_s4_reference_*/level3_s5_reference_*/
    level3_s6_reference_* come from `manifest` itself -- never re-derived.
    repository is always exactly REPOSITORY_IDENTITY."""
